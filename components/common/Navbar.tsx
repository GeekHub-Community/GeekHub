...
export const Navbar = () => {
    ...
    return (
        ...
        <div className='flex justify-between  w-full lg:max-w-[1600px] shadow fixed top-0  bg-blue  text-black p-4 z-[1] h-16'>
                        ...
                        <div className=' gap-5 mt-2  flex-col flex'>
                            {
                                Navlinks.map((link) => (
                                    <Link key={link.title} href={link.href}>
                                        <p className='text-base text-blue font-semibold hover:text-AthensGray/80  p-1 transition-all duration-200'>{link.title}</p>
                                    </Link>
                                ))
                            }
                        </div>
